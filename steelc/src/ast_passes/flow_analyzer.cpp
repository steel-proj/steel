...
void flow_analyzer::visit(function_declaration& func) {
	if (func.is_abstract()) {
		return;
	}
	...
	if (!current_returns && !func.return_type->is_void()) {
		...
		if (current_conditionally_returns) {
			HINT(HINT_CONDITIONAL_NOT_GUARANTEED_TO_RETURN);
		}
	}
...
void flow_analyzer::visit(code_block& block) {
	size_t i = 0;
...
void flow_analyzer::visit(return_statement& ret_stmt) {
...
void flow_analyzer::visit(break_statement& brk_stmt) {
	if (!in_loop) {
		ERROR(ERR_BREAK_OUTSIDE_LOOP, brk_stmt.span);
		return;
	}
}
void flow_analyzer::visit(if_statement& if_stmt) {
...
void flow_analyzer::visit(for_loop& for_loop) {
	bool was_in_loop = in_loop;
	in_loop = true;
	for_loop.body->accept(*this);
	in_loop = was_in_loop;
}
void flow_analyzer::visit(while_loop& while_loop) {
	bool was_in_loop = in_loop;
	in_loop = true;
