...
void declaration_collector::visit(compilation_unit& unit) {
...
void declaration_collector::visit(function_declaration& func_decl) {
	...
	if (flags & MOD_EXTERN && !func_decl.is_abstract()) {
		...
		HINT(HINT_USE_EXTERN_MODIFIER, func_decl.identifier.c_str());
	}
	...
	func_decl.owning_unit = current_unit;
...
void declaration_collector::visit(variable_declaration& var_decl) {
	...
	var_decl.owning_unit = current_unit;
	var_decl.parent_module = current_module;
}
void declaration_collector::visit(type_declaration& type_decl) {
	// validate modifiers
	modifier_flags flags = MOD_NONE;
	modifier_flags allowed = MOD_NONE;
	...
	type_decl.owning_unit = current_unit;
...
void declaration_collector::visit(module_declaration& mod_decl) {
	...
	mod_decl.owning_unit = current_unit;
	...
	if (module_ent != nullptr) {
		...
		module_entity* parent = current_module;
...
void declaration_collector::visit(enum_declaration& enum_decl) {
	...
	// set parent module & owning unit
	enum_decl.owning_unit = current_unit;
	enum_decl.parent_module = current_module;
}

void declaration_collector::visit(import_statement& import_stmt) {
	// ensure top-level
