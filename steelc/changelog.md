...
- Heavily changed the way the codegen backend system works by allowing the backend implementation to info about itself and using that to validate build configurations and provide better diagnostics in a codegen agnostic manner. This means the backend no longer has to worry about validation and can focus entirely on code generation, and the rest of the compiler can work with any backend that implements the required interface without having to worry about backend specific details.
